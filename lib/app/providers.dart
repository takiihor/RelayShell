...
import '../core/database/credentials_repository.dart';
import '../core/database/forwards_repository.dart';
...
import '../core/database/trusted_keys_repository.dart';
...
import '../core/ssh/forwarding_service.dart';
...
final scopedCommandsProvider =
  ...
  (ref, query) => ref.watch(commandsRepositoryProvider).watchVisibleIn(
        hostId: query.hostId,
...
final sessionsForHostProvider =
  ...
  (ref, hostId) => ref.watch(sessionsRepositoryProvider).watchForHost(hostId),
);
...
final forwardProfilesForHostProvider =
  ...
  (ref, hostId) => ref.watch(forwardsRepositoryProvider).watchForHost(hostId),
);
...
final hostConnectionStatusProvider =
  ...
  ref.watch(connectionChangesProvider);
  final connection = ref.watch(connectionManagerProvider).connectionFor(hostId);
  if (connection == null) {
    return Stream.value(
...
final hostReachabilityProvider =
    Provider.family<HostReachability, String>((ref, hostId) {
...
final activeForwardsProvider =
    StreamProvider<List<ActiveForward>>((ref) async* {
